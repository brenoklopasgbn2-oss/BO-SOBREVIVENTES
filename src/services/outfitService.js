...
import { findActiveStreamerCode } from './supportService.js';
...
import { currentPublicVipOutfitSlugsV613 } from '../data/vipOutfitsV201.js';
...
function normalizeAttachment(item, index = 0) {
  ...
  if (typeof item === 'string') {
    // Suporta cadeia simples de acoplamento:
    // Bainha@@1@>CombatKnife@@1@
    // O item depois de ">" é acoplado dentro do item anterior.
    const chain = item.split('>').map(part => part.trim()).filter(Boolean);
    const parsePart = (part, partIndex = 0) => {
      const [classname = '', slot = '', quantity = '1', liquidPercent = ''] = part.split('@').map(v => String(v || '').trim());
      if (!classname) return null;
      return {
        classname,
        slot: slot || null,
        quantity: Math.max(1, Math.min(Number(quantity || 1), 999)),
        liquidPercent: normalizePercent(liquidPercent),
        attachments: [],
        sortOrder: partIndex
      };
    };
    const root = parsePart(chain[0], index);
    if (!root) return null;
    let cursor = root;
    for (let i = 1; i < chain.length; i += 1) {
      const child = parsePart(chain[i], 0);
      if (!child) continue;
      cursor.attachments = [child];
      cursor = child;
    }
    return root;
  }
  ...
  return {
    classname,
    slot: String(item?.slot || '').trim() || null,
    quantity: Math.max(1, Math.min(Number(item?.quantity || 1), 999)),
    liquidPercent: normalizePercent(item?.liquidPercent ?? item?.fillPercent ?? item?.contentPercent),
    attachments: normalizeAttachments(item?.attachments ?? item?.attachedItems),
...
function normalizeAttachments(value) {
  ...
  return source.map((item, index) => normalizeAttachment(item, index)).filter(Boolean)
    .map((item, index) => ({ ...item, sortOrder: index }));
}

function attachmentToText(item) {
  const parts = [item.classname, item.slot || '', item.quantity || 1, item.liquidPercent ?? ''];
  while (parts.length > 1 && String(parts[parts.length - 1]) === '') parts.pop();
  const base = parts.join('@');
  const children = normalizeAttachments(item?.attachments ?? item?.attachedItems);
  // O painel usa ">" para representar um acoplado dentro de outro.
  // Para manter o formato leve e legível, cadeias são serializadas em profundidade.
  if (!children.length) return base;
  return `${base}>${attachmentToText(children[0])}`;
}

function attachmentsToText(value) {
  return normalizeAttachments(value).map(attachmentToText).join(',');
}
  ...
  return source
    .map((item, index) => {
      ...
      return {
        ...
        classname,
        quantity: Math.max(1, Math.min(Number(item?.quantity || 1), 999)),
        label: String(item?.label || classname).trim() || classname,
        liquidPercent: normalizePercent(item?.liquidPercent ?? item?.fillPercent ?? item?.contentPercent),
        attachments: normalizeAttachments(item?.attachments ?? item?.attachedItems),
        sortOrder: Number.isFinite(Number(item?.sortOrder)) ? Number(item.sortOrder) : index
      };
...
export function parseOutfitItemsText(text = '') {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (parts.length >= 4) {
      [slot, classname, quantity, label, liquidPercent = '', attachmentsText = ''] = parts;
    ...
    items.push({
      ...
      liquidPercent: normalizePercent(liquidPercent),
      attachments: normalizeAttachments(attachmentsText),
      sortOrder: index
    });
  }
  if (!items.length) {
    throw new Error('Adicione pelo menos 1 type. Formato: slot|classname|qtd|nome|liquido%|acoplados');
  }
...
export function outfitItemsToText(items = []) {
  return normalizeItems(items)
    .map(item => `${item.slot || 'inventory'}|${item.classname}|${item.quantity || 1}|${item.label || item.classname}|${item.liquidPercent ?? ''}|${attachmentsToText(item.attachments)}`)
    .join('\n');
}
...
export async function listOutfitTemplates({ includeInactive = false, includePrivate = false, serverType = 'vanilla', catalogOnly = false } = {}) {
  const server = normalizeServerType(serverType);
  const rows = await prisma.outfitTemplate.findMany({
    where: {
      ...(includeInactive ? {} : { active: true }),
      ...(includePrivate ? {} : { isPrivate: false }),
      slug: catalogOnly
        ? { in: currentPublicVipOutfitSlugsV613 }
        : { notIn: REMOVED_OUTFIT_SLUGS },
      OR: [{ serverType: server }, { serverType: 'all' }]
    },
    omit: { imageData: true },
    orderBy: [{ isPrivate: 'asc' }, { level: 'asc' }, { priceCoins: 'asc' }, { name: 'asc' }]
  });
  return rows.map(row => ({ ...row, hasImageData: Boolean(row.imageMime) }));
}

export async function getActiveOutfitForPlayer(steam64, serverType = 'vanilla') {
  const server = normalizeServerType(serverType);
  const cleanSteam64 = String(steam64 || '').trim();
  if (/^\d{17}$/.test(cleanSteam64)) {
    await syncClanManagedOutfitForSteam64(cleanSteam64);
  }
  const now = new Date();
  ...
  const sub = await prisma.playerOutfitSubscription.findFirst({
    ...
    include: { outfitTemplate: { omit: { imageData: true } } },
...
function outfitHas80SlotAssaultPack(outfit) {
  const name = String(outfit?.name || '').toLowerCase();
  if (name.includes('assault pack 80 slots')) return true;
  const items = normalizeItems(outfit?.items);
  return items.some(item => {
    const slot = String(item?.slot || '').toLowerCase();
    const classname = String(item?.classname || '').toLowerCase();
    const label = String(item?.label || '').toLowerCase();
    return ['back','backpack'].includes(slot)
      && classname.includes('assault_pack')
      && (label.includes('80') || name.includes('80 slots'));
  });
}

export async function getStarterVipBenefitStatus(playerId) {
  if (!playerId) return { eligible: false, used: false, reason: 'Faça login pelo jogo.' };
  const [claim, used] = await Promise.all([
    prisma.starterKitClaim.findFirst({ where: { playerId }, select: { id: true, claimedAt: true } }),
    prisma.playerOutfitSubscription.findFirst({
      where: { playerId, source: 'STARTER_KIT_7D' },
      orderBy: { createdAt: 'asc' },
      select: { id: true, createdAt: true, expiresAt: true, outfitTemplateId: true }
    })
  ]);
  if (!claim) return { eligible: false, used: false, reason: 'Resgate o Kit Inicial para liberar o VIP grátis.' };
  if (used) return { eligible: false, used: true, reason: 'O VIP grátis do Kit Inicial já foi usado.', usedSubscription: used };
  return { eligible: true, used: false, claimedAt: claim.claimedAt, backpackSlots: 80, durationDays: 7 };
}

export async function claimStarterVip7d({ playerId, outfitId }) {
  return prisma.$transaction(async (tx) => {
    const player = await tx.player.findUnique({ where: { id: playerId } });
    if (!player) throw new Error('Player não encontrado.');

    const starterClaim = await tx.starterKitClaim.findFirst({ where: { playerId }, select: { id: true } });
    if (!starterClaim) throw new Error('Resgate o Kit Inicial antes de escolher o traje VIP grátis.');

    const alreadyUsed = await tx.playerOutfitSubscription.findFirst({
      where: { OR: [{ playerId }, { steam64: player.steam64 }], source: 'STARTER_KIT_7D' },
      select: { id: true }
    });
    if (alreadyUsed) throw new Error('Você já utilizou o traje VIP grátis de 7 dias do Kit Inicial.');

    const privateVip = await findActivePrivateVip(tx, { playerId });
    if (privateVip) throw new Error(`Você possui o VIP privado "${privateVip.outfitTemplate.name}" e não pode ativar o VIP grátis normal.`);

    const outfit = await tx.outfitTemplate.findUnique({ where: { id: outfitId } });
    if (!outfit || !outfit.active || outfit.isPrivate) throw new Error('Traje VIP não encontrado ou indisponível.');
    if (!outfitHas80SlotAssaultPack(outfit)) {
      throw new Error('O benefício grátis de 7 dias só pode ser usado no traje com Assault Pack de 80 slots.');
    }

    await tx.playerOutfitSubscription.updateMany({
      where: { playerId, status: 'ACTIVE', source: { in: ['PURCHASE', 'STARTER_KIT_7D'] } },
      data: { status: 'CANCELLED' }
    });

    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    const subscription = await tx.playerOutfitSubscription.create({
      data: {
        playerId,
        steam64: player.steam64,
        outfitTemplateId: outfit.id,
        serverType: outfit.serverType,
        source: 'STARTER_KIT_7D',
        expiresAt
      }
    });
    ...
    await logAudit({
      ...
      action: 'outfit.starter_kit_7d.claimed',
      target: outfit.id,
      data: { outfitName: outfit.name, expiresAt, backpackSlots: 80, priceCoins: 0 }
    });

    return { player, outfit, subscription, expiresAt };
  });
}
...
export async function upsertOutfitTemplateFromBody({ body, file, id = null }) {
  ...
  const data = {
    ...
    imageUrl: String(body.videoUrl || body.mediaUrl || body.imageUrl || '').trim() || null,
    items,
...
export async function revokeOutfitSubscriptionByAdmin({ subscriptionId, actor = 'admin' }) {
  ...
  const steam64 = String(subscription.steam64 || subscription.player?.steam64 || '').trim();
  if (!/^\d{17}$/.test(steam64)) throw new Error('Steam64 inválido no cadastro VIP.');

  // Remover pelo painel ADM significa retirar o VIP do player por completo.
  // Antes apenas a linha clicada era cancelada; se existisse outro VIP ativo
  // para a mesma Steam64, ele assumia o cache local do mod e parecia que a remoção
  // não tinha funcionado.
  const cancelled = await prisma.playerOutfitSubscription.updateMany({
    where: {
      steam64,
      status: 'ACTIVE'
    },
  ...
  // Garante que a linha clicada também fique cancelada mesmo se já estivesse
  // vencida/fora do filtro ACTIVE por alguma inconsistência antiga.
  ...
  await logAudit({
    ...
    data: {
      ...
      cancelledActiveCount: cancelled.count,
      removeAllActiveForSteam64: true
    }
  });
  ...
  return {
    ...updated,
    steam64,
    revokedCount: cancelled.count,
    player: subscription.player,
    outfitTemplate: subscription.outfitTemplate
  };
}
