...
export async function logPaymentApproved({ player, payment }) {
  const supportField = payment.supportStreamerCode
    ...
    : [];
  const embed = {
    author: { name: `${env.appName} • PIX aprovado`, icon_url: DISCORD_STYLE.pixIcon },
    ...
    footer: { text: `${env.appName} • Sistema automático de doações` },
    timestamp: new Date().toISOString()
  };
  await sendDiscord({ embeds: [embed] }, { kind: 'sales', username: `💸 ${env.appName} • Doações` });
  if (env.discordSalesWebhookUrl && env.discordWebhookUrl && env.discordSalesWebhookUrl !== env.discordWebhookUrl) {
    await sendDiscord({ embeds: [embed] }, { kind: 'default', username: `💸 ${env.appName} • Doações` });
  }
}
...
export async function logPurchase({ player, product, purchase, delivery, deliveries = [], coupon = null, support = null, gift = null }) {
  return sendDiscord({
    embeds: [{
      author: { name: `${env.appName} • Loja Vanilla`, icon_url: DISCORD_STYLE.storeIcon },
      ...
      fields: [
        ...
        ...(coupon?.code ? [{ name: '🏷️ Cupom aplicado', value: `**${coupon.code}**\nDesconto: **-${coins(coupon.discountCoins)} ${env.currencyName}**`, inline: true }] : []),
        ...(support?.code ? [{ name: '❤️ Streamer apoiado', value: `**${support.streamerName}**\nCódigo: **${support.code}**\nComissão: **${coins(support.commissionCoins)} ${env.currencyName}**`, inline: true }] : []),
        ...(gift?.steam64 ? [{ name: '🎁 Presente para', value: `\`${gift.steam64}\``, inline: true }] : [])
      ],
      footer: { text: `Compra ${shortId(purchase.id)} • ${env.appName}` },
      timestamp: new Date().toISOString()
    }]
  }, { kind: 'sales', username: `🛒 ${env.appName} • Vendas` });
}

export async function logCartPurchase({ player, purchases = [], deliveries = [], totalCoins = 0, coupon = null, support = null }) {
  const productsLine = purchases.map((p, idx) => `**${idx + 1}.** ${p.productName || p.product?.name || 'Produto'} — ${p.quantity}x — **${coins(p.totalCoins)} ${env.currencyName}**`).join('\n').slice(0, 1000) || '-';
  return sendDiscord({
    embeds: [{
      author: { name: `${env.appName} • Carrinho finalizado`, icon_url: DISCORD_STYLE.cartIcon },
      ...
      footer: { text: `${env.appName} • Carrinho e cupom` },
      timestamp: new Date().toISOString()
    }]
  }, { kind: 'sales', username: `🧺 ${env.appName} • Carrinho` });
}
...
export async function sendRankingToDiscord({ rankingData, kind = 'players', server = 'vanilla', period = 'weekly', title = '' }) {
  ...
  const embeds = chunks.slice(0, 10).map((chunk, idx) => ({
    author: idx === 0 ? { name: `${env.appName} • Ranking ${labelServer}`, icon_url: DISCORD_STYLE.trophyIcon } : undefined,
    ...
    footer: { text: `${env.appName} • Ranking automático completo` },
    timestamp: new Date().toISOString()
  }));
