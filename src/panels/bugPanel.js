...
function buildBugPanel() {
  ...
  const embed = baseEmbed()
    ...
    .setDescription([
      ...
      '',
      'Abra o ticket abaixo e descreva exatamente o que aconteceu, como reproduzir e anexe prints ou vídeos.'
    ].join('\n'))
    .setImage(`attachment://${imageName}`)
    .addFields({ name: 'Dica', value: 'Quanto mais detalhes, mais rápido a equipe consegue corrigir.', inline: false });

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(TICKET_TYPES.bug.customId)
      .setLabel(TICKET_TYPES.bug.label)
      .setEmoji(TICKET_TYPES.bug.emoji)
