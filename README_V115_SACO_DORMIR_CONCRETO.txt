RAID-Z LOJA V115 - SACO DE DORMIR E CONCRETO

ADICIONADO:
- Categoria pública "Saco de Dormir".
- Saco Azul: DDTB_sleepingbag_Blue por 40.000 RZ.
- Saco Verde: DDTB_sleepingbag_Green por 40.000 RZ.
- Sem limite/espera de 15 dias para comprar.
- Painel de regras visível na categoria.
- Máximo de 1 saco ativo por player.
- Regra atual ZONA-Z: Sleeping Bag pode ficar dentro da própria base principal; não existe obrigação de deixá-lo fora.
- Base No Raid: pode usar na base principal, sem FOB.
- Player encontrado com 2 sacos recebe ban.
- Aviso de sacos gratuitos nos bunkers.
- Aviso de que o player pode morrer/respawnar no jogo para trocar spawn; spawn Vanilla melhorado.
- Mais informações no Discord.

CONCRETO:
- Produto Blocos de Concreto x5.
- Classname: ConcreteBlock.
- Quantidade: 5.
- Valor: 15.000 RZ.
- Categoria: Construção.

Não foi criada regra de compra a cada 15 dias.
Não possui migration nova e não apaga dados existentes.
